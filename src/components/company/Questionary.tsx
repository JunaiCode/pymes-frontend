...
const Questionary = (props: any) => {
    const [evaluationResults, setevaluationResults] = useState([
        ...
        {
            id: 5,
            title: "¿Qué tan importante es la personalización de la experiencia del cliente?",
            options: [
                { title: "No es importante." },
                { title: "Es importante." },
                { title: "Es muy importante." },
    ...
    const [progress, setProgress] = useState({ total: evaluationResults.length, completed: evaluationResults.filter((question) => question.answer !== null).length });
    ...
    useEffect(() => {
        ...
        const getEvaluationResult = async () => {
            const evaluationResults = await fetch(`${baseUrl}/evaluation/${companyId}`);
    ...
    useEffect(() => {
        setIndex(evaluationResults.findIndex((question: any) => question.id === currentQuestion.id));
    ...
    const questionsToShow = evaluationResults.slice(currentPage * questionsPerPage, (currentPage + 1) * questionsPerPage);
    return (
        !resultsScreen ? (
                        ...
                        <div key={index} className="mt-24 flex flex-col h-5/4 bg-white mr-2 border rounded-lg shadow-lg p-12 w-5/6">
                            <div className="flex justify-end">
                                <button onClick={handleMarkQuestion} className="bg-secondary_old text-white p-2 mb-4 rounded shadow-lg">
                                    {currentQuestion.marked ? 'Desmarcar Pregunta' : 'Marcar Pregunta'}
                                </button>
                            </div>
                            ...
                            <div className="flex flex-col justify-center items-center gap-6">
                                ...
                                <IoChevronBack size={48} onClick={handlePreviousQuestion} className={`text-primary text-4xl cursor-pointer`} />
                                    {questionsToShow.map((question: any, questionIndex: any) => (
                                        <div key={questionIndex}>
                                            ...
                                            <hr className={` h-0.5 w-full mt-4 ${question.id === currentQuestion.id? 'bg-primary':''}`}/>                                           
                                        </div>
                                    ))}
                                    <IoChevronForward  size={48} onClick={handleNextQuestion} className="text-primary text-4xl cursor-pointer" />
                                </div>
                                {finished && (
