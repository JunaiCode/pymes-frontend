...
export const RecommendationComponent = ({description,tag,index,dimension,handleCheck}:RecommendationProps) => {
    ...
    return(
        ...
        <div className="flex flex-row justify-center items-baseline gap-2">
            {hidden?<FaAngleDown className="text-primary cursor-pointer" size={24} onClick={()=>{
        ...
        }/>:<FaAngleUp className="text-primary cursor-pointer" size={24} onClick={()=>{let ul = document.getElementById(`${dimension}-${index}`);ul?.classList.add('hidden'); setHidden(true);}
    }/>}
        ...
        <ul id={`${dimension}-${index}`} className={`${hidden?'hidden':''} ml-8`}>
